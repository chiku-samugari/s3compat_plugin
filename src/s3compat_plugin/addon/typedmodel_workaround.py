...
class TypedModelRegistrationMixin:
    """
    Usage:
        ...
        class MyFile(TypedModelRegistrationMixin, MyFileNode, File):
            class Meta:
                proxy = True
                app_label = 'myaddon'

        # After all model definitions are complete (e.g., in apps.py ready() method):
        MyFile.register_in_typedmodel_registry()
    """

    @classmethod
    def register_in_typedmodel_registry(cls):
        """
...
def register_all_models(*model_classes):
    """
    ...
    Args:
        *model_classes: Model classes that use TypedModelRegistrationMixin
    """
    for model_cls in model_classes:
        if not hasattr(model_cls, 'register_in_typedmodel_registry'):
            raise TypeError(
                f"{model_cls.__name__} does not have register_in_typedmodel_registry. "
                f"Did you forget to inherit from TypedModelRegistrationMixin?"
            )
        model_cls.register_in_typedmodel_registry()
